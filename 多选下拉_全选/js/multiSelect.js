	...
	multiSelect.prototype.init = function() {
		...
		o('select[multiple]').each(function(idx, item) {
			...
			t.next().addClass('multi').find('.layui-select-title').click(function() {
				...
				if(dt.attr('lay-value') === '全选' && !dt.find('input').prop('checked')){
					dt.nextAll().each(function(){
						if(o(this).find('input').prop('checked')){o(this).click()}
					})
				}
				if(dt.attr('lay-value') === '全选' && dt.find('input').prop('checked')){
					dt.nextAll().each(function(){
				...
				if(dt.attr('lay-value') !== '全选'){
					var dtparentEq=dt.parent().find('input').eq(0);
					if(dt.find('input').prop('checked')){
						n++;
						if(n===dt.siblings().length){
							dtparentEq.prop('checked',true);
							dtparentEq.next().addClass('layui-form-checked');
							}
					}else{
						n--;
						if(n!==dt.siblings().length){
							dtparentEq.prop('checked',false);
							dtparentEq.next().removeClass('layui-form-checked');
						}
					}
				}
				// 禁止下拉框收回
